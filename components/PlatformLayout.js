...
const ThemeSwitcher = () => {
    ...
    return (
        <button onClick={toggleTheme} className="w-full flex items-center justify-between text-left px-4 py-2.5 rounded-lg transition text-md font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700">
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-sans text-slate-800 dark:text-slate-200">
            ...
            <aside className={`fixed inset-y-0 left-0 bg-white dark:bg-slate-800 w-80 p-6 h-screen shadow-2xl flex flex-col transform transition-transform duration-300 z-40 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
                ...
                <div className="flex items-center justify-between mb-6 border-b border-slate-200 dark:border-slate-700 pb-4">
                    ...
                    <button onClick={handleLogout} disabled={isLoggingOut} className="p-2 rounded-lg text-red-500 hover:bg-red-100 dark:hover:bg-red-900/50 disabled:opacity-70">{isLoggingOut ? <div className="loader h-5 w-5"></div> : <Icons.LogoutIcon />}</button>
                </div>
                ...
                <div className="mt-6 border-t border-slate-200 dark:border-slate-700 pt-4"><ThemeSwitcher /></div>
            </aside>
