...
// Versão com funcionalidade de minimizar, pop-ups de novas mensagens e controle de notificações.
...
// 👇 IMPORTS do ChatUI (confira se o arquivo existe em: components/chat/ChatUI.js)
import {
  SendIcon,
  AiIcon,
  UsersIcon,
...
const GlobalChat = ({ isVisible, onClose }) => {
    ...
    script.onload = () => {
      const startAudio = () => {
        ...
        notificationSound.current =
          new window.Tone.Synth().toDestination();
  ...
  // 🔹 Carregar mensagens + usuários online
  useEffect(() => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (
          ...
          (!lastMessageTimestamp ||
            msgData.timestamp > lastMessageTimestamp)
        ) {
          hasNewMessage = true;
      ...
      if (
        ...
        if (isMinimized && popupsEnabled) {
          ...
          if (notificationSound.current) {
            notificationSound.current.triggerAttackRelease(
              'C5',
              '8n'
            );
          }
    ...
    const unsubscribeStatus = onValue(statusRef, (snapshot) => {
      ...
      if (statuses) {
        for (const uid in statuses) {
          if (
            statuses[uid].state === 'online' ||
            statuses[uid].state === 'busy'
          ) {
      ...
      <div className="fixed bottom-4 right-4 z-20">
        {newNotification && (
          <div className="absolute bottom-20 right-0 w-72 bg-white dark:bg-slate-700 p-3 rounded-lg shadow-xl animate-fade-in">
            <p
              className={`font-bold text-sm ${getRoleColor(
  ...
  return (
    <div className="fixed bottom-4 right-4 w-96 h-[600px] flex flex-col bg-white dark:bg-indigo-900 rounded-lg shadow-2xl z-20 animate-fade-in">
      {isOnlineListVisible && (
        <OnlineUsersModal
          users={onlineUsers}
          onClose={() => setIsOnlineListVisible(false)}
      ...
      <header className="p-4 border-b border-slate-200 dark:border-indigo-800 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() =>
              updateUserChatStatus(
                chatStatus === 'online' ? 'busy' : 'online'
              )
            }
            className={`px-3 py-1.5 text-sm font-medium rounded-full transition ${
              chatStatus === 'busy' ? 'bg-yellow-500' : 'bg-green-500'
            } text-white`}
          >
            {chatStatus === 'busy' ? 'Ocupado' : 'Disponível'}
          </button>
          <button
            onClick={() => {
          ...
          <button
            onClick={() => setIsOnlineListVisible(true)}
            className="p-1.5 bg-slate-100 dark:bg-indigo-800 rounded-full text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-indigo-700 transition"
          >
            <UsersIcon />
          </button>
          <button
            onClick={() => setIsMinimized(true)}
            className="p-1.5 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-indigo-700 rounded-full transition"
            title="Minimizar"
          >
            <MinimizeIcon />
          </button>
            ...
            <div
              className={`p-3 rounded-2xl max-w-lg ${
                msg.uid === user.uid
                  ...
                  : 'bg-slate-100 dark:bg-indigo-800 rounded-bl-none'
              }`}
            >
              <p
                className={`font-bold text-sm mb-1 ${getRoleColor(
                  msg.role
                )} ${msg.uid === user.uid ? 'hidden' : 'block'}`}
              >
                {msg.name}
              </p>
      ...
      <footer className="p-4 border-t border-slate-200 dark:border-indigo-800">
        <div className="flex items-center justify-center mb-2">
          <label
            htmlFor="popups-toggle"
            className="flex items-center cursor-pointer text-xs text-slate-500 dark:text-slate-400"
          >
            <input
              type="checkbox"
              id="popups-toggle"
        ...
        >
          <button
            ...
            className={`p-2 rounded-full transition ${
              isAiSelected
                ...
                : 'bg-slate-200 dark:bg-indigo-800 text-slate-500 dark:text-slate-300'
            }`}
          ...
          <button
            type="submit"
            className="p-2.5 font-semibold text-white bg-blue-600 rounded-full hover:bg-blue-700 transition disabled:bg-slate-400"
            disabled={isLoading}
          >
            <SendIcon />
          </button>
