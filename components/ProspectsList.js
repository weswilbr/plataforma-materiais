...
// COMPONENTE ATUALIZADO: Adicionado campo de email; Telefone e Email são opcionais.
...
const AddProspectForm = ({ onAdd, isLoading }) => {
    ...
    const handleSubmit = (e) => {
        ...
        onAdd({ name, whatsapp, phone, email });
        setName('');
        setWhatsapp('');
        setPhone('');
        setEmail('');
...
const EditProspectModal = ({ prospect, onSave, onClose, isLoading }) => {
    ...
    const handleSave = () => {
        ...
        onSave(prospect.id, { name, whatsapp, phone, email, status });
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-2xl w-full max-w-md">
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div><label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Email</label><input type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 focus:outline-none"/></div>
            ...
            <div className="bg-white dark:bg-indigo-900 shadow-lg rounded-lg overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-slate-500 dark:text-slate-400">
                        ...
                        <tbody>
                            {isLoading ? (
                                <tr><td colSpan="6" className="text-center p-8">A carregar lista...</td></tr>
                            ) : prospects.length > 0 ? (
                                prospects.map(p => (
                                <tr key={p.id} className="border-b dark:border-indigo-800 hover:bg-slate-50 dark:hover:bg-indigo-800/50">
                                    ...
                                    <td className="px-6 py-4">
                                        {p.email ? (
                                             <a href={`mailto:${p.email}`} className="flex items-center gap-2 text-slate-600 dark:text-slate-400 hover:underline">
                                                <EmailIcon /> {p.email}
                                            </a>
                                        ) : '-'}
                                    </td>
                                ...
                                <tr><td colSpan="6" className="text-center p-8">Ainda não há prospectos. Comece por adicionar um!</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
