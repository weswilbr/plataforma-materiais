...
// Versão atualizada para detetar convites na URL e mostrar o formulário de registo.
...
const LoginScreen = () => {
    ...
    const { login, signUpWithInvite, resetPassword, signInWithGoogle } = useAuth();
    ...
    useEffect(() => {
        if (router.query.invite) {
            ...
            setMode('signup');
            setMessage('Bem-vindo! Preencha os seus dados para aceitar o convite.');
        }
    }, [router.query]);
    ...
    const handleAuthAction = async (e) => {
        ...
        if (mode === 'login') {
            ...
            result = await signUpWithInvite(inviteCode, name, email, password);
            if(result.success) {
                setMessage("Conta criada com sucesso! Pode agora fazer login.");
                setMode('login');
            }
        } else if (mode === 'reset') {
            result = await resetPassword(email);
            if(result.success) setMessage("Email de recuperação enviado!");
        }
    ...
    const handleGoogleSignIn = async () => {
        ...
        // O onAuthStateChanged no AuthContext irá tratar do redirecionamento
        setIsLoading(false);
    }
        ...
        <div className="flex items-center justify-center min-h-screen bg-slate-100 dark:bg-indigo-950">
            <div className="w-full min-h-screen p-8 space-y-6 bg-white dark:bg-indigo-900 flex flex-col justify-center md:min-h-0 md:w-full md:max-w-md md:h-auto md:rounded-xl md:shadow-2xl">
                ...
                <form className="space-y-6" onSubmit={handleAuthAction}>
                    {mode === 'signup' && (
                        <div className="space-y-2">
                            ...
                            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-2 bg-slate-50 dark:bg-indigo-800 border border-slate-300 dark:border-indigo-700 rounded-lg focus:ring-2 focus:ring-blue-500 text-slate-900 dark:text-white" required />
                        </div>
                    )}
                    ...
                    {mode !== 'reset' && (
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-2 pr-10 bg-slate-50 dark:bg-indigo-800 border border-slate-300 dark:border-indigo-700 rounded-lg focus:ring-2 focus:ring-blue-500 text-slate-900 dark:text-white" required />
                                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 flex items-center px-3 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">
                ...
                <div className="space-y-4">
                    ...
                    <button onClick={handleGoogleSignIn} disabled={isLoading} className="w-full px-4 py-2 font-semibold text-slate-700 dark:text-white bg-white dark:bg-indigo-800 border border-slate-300 dark:border-indigo-700 rounded-lg hover:bg-slate-50 dark:hover:bg-indigo-700 transition flex items-center justify-center gap-2">
                ...
                <div className="text-sm text-center">
                    {mode === 'login' ? (
                        <p className="text-slate-600 dark:text-slate-400">Não tem um convite? <button onClick={() => { setMode('reset'); setError(''); }} className="font-medium text-slate-500 hover:underline dark:text-slate-400">Esqueceu-se da senha?</button></p>
                    ) : (
                        <p className="text-slate-600 dark:text-slate-400">Já tem uma conta? <button onClick={() => { setMode('login'); setError(''); setMessage(''); }} className="font-medium text-blue-600 hover:underline dark:text-blue-400">Faça login</button></p>
                    )}
                </div>
            </div>
        </div>
